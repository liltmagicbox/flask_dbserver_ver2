...
addn(user,no,key) keys=[조회,추천,비추,즐찾]
addtag(user,no,key,text) keys=[태그,유저태그]
addcomm(user,no,key,text) keys=[댓글]

onlykey = [태그,유저태그]
if key in unique: isunique(key,text)

fluid[no][key]={ 1:{}, 2:{}, }
if len( fluid[no][key] ) == 0: idx=0
else: idx = max( fluid[no][key].keys() )+1 #starting 1
fluid[no][key][idx] = { 'user':user 'text':text 'time':time 'see':see }

del fluid[no][key][item][idx]

len(fluid[133][조회]) #is only for num.fine.
...
add(user,no,key,text,same)
add(user,no,key,text,same=False) for text:tag,comment,require
fluid[번호][조회]=[ 해시:{text='',user,date} ]
fluid[번호][댓글]=[ 해시:{text,user,date} ]
for i :순차,무작위접근
[해시]: 지정접근
sub(user,no,key,해시)
sub(user,no,key,해시)
삭제는없다.
..라기엔 관리자삭제는필요한데.
애초에 삭제를, 해당키가 아닌 넘버,키,텍스트로 들어가는게 어처구니없는것이야.
